...
rmarkdown::render('vignettes/build_metageo_instance.Rmd')
```
...
source('sjogren_info.R')
```

## metaGEO Rdata generation configurations
The `GEOmetadb.sqlite` file can be downloaded from [https://gbnci-abcc.ncifcrf.gov/geo/]
...
```{r 'match genes to bioconductor annotation', eval=T, warning=T}
